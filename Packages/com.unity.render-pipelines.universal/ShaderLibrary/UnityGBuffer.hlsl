...
#define kMaterialFlagSimpleLit                16 // Indicates the material is simple lit (for GDRP)
...
FragmentOutput SurfaceDataToGbuffer(SurfaceData surfaceData, InputData inputData, half3 globalIllumination, int lightingMode UNITY_GDRP_INSTANCE_PARAMETER)
{
    ...
    #if defined(SIMPLE_LIT_GBUFFER_SPLAT)
    materialFlags |= kMaterialFlagSimpleLit;
    #endif
    ...
    uint renderingLayers = instance.renderingLayerMask;
    #else
...
// This will encode SurfaceData into GBuffer
FragmentOutput BRDFDataToGbuffer(BRDFData brdfData, InputData inputData, half smoothness, half3 globalIllumination UNITY_GDRP_INSTANCE_PARAMETER, half occlusion = 1.0)
{
    half3 packedNormalWS = PackNormal(inputData.normalWS);
