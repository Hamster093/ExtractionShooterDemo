...
// Transforms normal from object to world space
float3 TransformObjectToWorldNormalGDRP(float3 normalOS, float4x4 worldToObjectMatrix, bool doNormalize = true)
{    
    // Normal need to be multiply by inverse transpose
...
// tangentToWorld is the matrix representing the transformation of a normal from tangent to world space
real3 TransformObjectToTangentGDRP(real3 dirOS, real3x3 tangentToWorld, float4x4 worldToObjectMatrix)
{
    // Note matrix is in row major convention with left multiplication as it is build on the fly

    // don't normalize, as normalWS will be normalized after TransformWorldToTangent
    float3 normalWS = TransformObjectToWorldNormalGDRP(dirOS, worldToObjectMatrix, false);
