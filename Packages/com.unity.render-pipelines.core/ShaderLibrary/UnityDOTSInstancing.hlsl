...
TEXTURE2D_FLOAT(unity_InstanceDataMap);
SAMPLER(samplerunity_InstanceDataMap) { Filter = MIN_MAG_MIP_POINT; AddressU = Clamp; AddressV = Clamp; };
...
static const uint kIndirectMask       = 0x0fffffff;
...
#if defined(USE_VTF)
    dotsInstanceIndex = unity_DOTSVisibleInstances[unity_InstanceID].VisibleData.x;

    if(crossFadeValuePacked != 0)
    {
        visibleData.VisibleData.y = int(dotsInstanceIndex) >> 24;
#if defined(SHADER_API_METAL)
        visibleData.VisibleData.y |= (dotsInstanceIndex & 0x80000000) ? 0xffffff00 : 0x0;
#endif
        dotsInstanceIndex &= 0x00ffffff;
    }
    else
    {
        visibleData.VisibleData.y = 0;
    }
#else
    uint packedIndirectVisibleOffset = unity_DOTSVisibleInstances[0].VisibleData.y;
    //uint dotsIndirectVisibleIndex = (packedIndirectVisibleOffset & ~kIndirectVisibleOffsetEnabledBit) + unity_InstanceID;

    //To deal with Qualcomm bug( big mask value can not retrieve correct value from buffer), we use a smaller mask here
    uint dotsIndirectVisibleIndex = (packedIndirectVisibleOffset & kIndirectMask) + unity_InstanceID;

    if (packedIndirectVisibleOffset != 0)
        dotsInstanceIndex = LoadDOTSIndirectInstanceIndex(dotsIndirectVisibleIndex);
    else
        dotsInstanceIndex = unity_DOTSVisibleInstances[unity_InstanceID].VisibleData.x;
...
#endif
    visibleData.VisibleData.x = dotsInstanceIndex;

    SetDOTSVisibleData(visibleData);
}
...
static uint unity_DOTSInstanceData_VTFStride;
static uint unity_DOTSInstanceData_VTFWidthMask;
static uint unity_DOTSInstanceData_VTFWidthBitCount;

static uint unity_DOTSInstanceData_Texture_Select4_Mask0;
static uint unity_DOTSInstanceData_Texture_Select4_Mask1;
static uint unity_DOTSInstanceData_Texture_Select2_Mask;

void SetupDOTSInstanceVTFStride(uint stride)
{
    const uint strideMask = 0x00ffffff;
    const uint widthMask = 0xff000000;
    unity_DOTSInstanceData_VTFStride = stride & strideMask;
    unity_DOTSInstanceData_VTFWidthBitCount = (stride & widthMask) >> 24;
    unity_DOTSInstanceData_VTFWidthMask = (1 << unity_DOTSInstanceData_VTFWidthBitCount) - 1;
    ...
    // NOTE: Mask generation was rewritten to this form specifically to avoid codegen
    // correctness issues on GLES.
    unity_DOTSInstanceData_Texture_Select4_Mask0 = (offsetSingleChannel & 0x4) ? 0xffffffff : 0;
    unity_DOTSInstanceData_Texture_Select4_Mask1 = (offsetSingleChannel & 0x8) ? 0xffffffff : 0;
    // Select2 mask is the same as the low bit mask of select4, since
    // (x << 3) << 28 == (x << 2) << 29
    unity_DOTSInstanceData_Texture_Select2_Mask = unity_DOTSInstanceData_Texture_Select4_Mask0;
}

uint DOTSInstanceData_Select(uint addressOrOffset, uint4 v)
{
    uint mask0 = unity_DOTSInstanceData_Texture_Select4_Mask0;
    uint mask1 = unity_DOTSInstanceData_Texture_Select4_Mask1;
    return
...
{
    uint mask0 = unity_DOTSInstanceData_Texture_Select2_Mask;
    return (v.zw & mask0) | (v.xy & ~mask0);
}

uint DOTSInstanceData_Load(uint address)
{
    uint texAddress = address >> 4;
    uint4 texRaw = asuint(LOAD_TEXTURE2D_LOD(unity_InstanceDataMap, int2(texAddress & unity_DOTSInstanceData_VTFWidthMask, texAddress >> unity_DOTSInstanceData_VTFWidthBitCount), 0));
    return DOTSInstanceData_Select(address, texRaw);
}
...
uint2 DOTSInstanceData_Load2(uint address)
{
    uint texAddress = address >> 4;
    uint4 texRaw = asuint(LOAD_TEXTURE2D_LOD(unity_InstanceDataMap, int2(texAddress & unity_DOTSInstanceData_VTFWidthMask, texAddress >> unity_DOTSInstanceData_VTFWidthBitCount), 0));
    return DOTSInstanceData_Select2(address, texRaw);
}

uint4 DOTSInstanceData_Load4(uint address)
{
    // ensure address low four bits are zero
    uint texAddress = address >> 4;
    float4 texRaw = LOAD_TEXTURE2D_LOD(unity_InstanceDataMap, int2(texAddress & unity_DOTSInstanceData_VTFWidthMask, texAddress >> unity_DOTSInstanceData_VTFWidthBitCount), 0);
    return asuint(texRaw);
}
    ...
    case 8:
        {
            ...
            uint4 raw0 = DOTSInstanceData_Load4(address);
            uint4 raw1 = DOTSInstanceData_Load4(address + 16);
            uint3 v;
            v.xy = raw0.zw;
            v.z  = raw1.x;
            return v;
...
#elif defined(UNITY_DOTS_INSTANCING_UNIFORM_BUFFER)
uint DOTSInstanceData_Select(uint addressOrOffset, uint4 v)
{
...
float4x4 LoadObjectToWorldMatrixFromTex(uint addr)
{
    addr = ComputeDOTSInstanceDataAddress(addr, 16 * 3);
    uint rawAddress = addr;

    float4 p1 = asfloat(DOTSInstanceData_Load4(rawAddress + 0 * 16));
    float4 p2 = asfloat(DOTSInstanceData_Load4(rawAddress + 1 * 16));
    float4 p3 = asfloat(DOTSInstanceData_Load4(rawAddress + 2 * 16));
    // float4 p4 = asfloat(DOTSInstanceData_Load4(rawAddress + 3 * 16));
    // return float4x4(
    //     p1.x, p2.x, p3.x, p4.x,
    //     p1.y, p2.y, p3.y, p4.y,
    //     p1.z, p2.z, p3.z, p4.z,
...
float4x4 LoadWorldToObjectMatrixFromTex(uint addr)
{
    addr = ComputeDOTSInstanceDataAddress(addr, 16 * 3);
    uint rawAddress = addr;
...
float4 LoadLightMapFromTex(uint addr)
{
    addr = ComputeDOTSInstanceDataAddress(addr, 16);
    uint rawAddress = addr;

    return asfloat(DOTSInstanceData_Load4(rawAddress));
}



void SetupDOTSSHCoeffsFromTex(uint shMetadata)
{
    if (IsDOTSInstancedProperty(shMetadata))
    {
        uint addr = ComputeDOTSInstanceDataAddress(shMetadata, 16 * 8);
        uint rawAddress = addr;

        unity_DOTS_Sampled_SHAr = asfloat(DOTSInstanceData_Load4(rawAddress + 0 * 16));
        unity_DOTS_Sampled_SHAg = asfloat(DOTSInstanceData_Load4(rawAddress + 1 * 16));
        unity_DOTS_Sampled_SHAb = asfloat(DOTSInstanceData_Load4(rawAddress + 2 * 16));
        unity_DOTS_Sampled_SHBr = asfloat(DOTSInstanceData_Load4(rawAddress + 3 * 16));
        unity_DOTS_Sampled_SHBg = asfloat(DOTSInstanceData_Load4(rawAddress + 4 * 16));
        unity_DOTS_Sampled_SHBb = asfloat(DOTSInstanceData_Load4(rawAddress + 5 * 16));
        unity_DOTS_Sampled_SHC  = asfloat(DOTSInstanceData_Load4(rawAddress + 6 * 16));
        unity_DOTS_Sampled_ProbesOcclusion = asfloat(DOTSInstanceData_Load4(rawAddress + 7 * 16));
    }
    else
    {
